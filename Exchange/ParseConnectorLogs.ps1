...
ForEach ($Server in $ExCAS) {
    ...
    $ServerLogs = Get-ChildItem -Path $CASPath *.log | Sort-Object -Property LastWriteTime | select -Last 5

    If ($ServerLogs) {
	Write-Host -Foreground Yellow "$CASPath"
	$ServerLogs | ForEach-Object -Process {
	    Write-Host ("< {0}" -f $_) -ForegroundColor Yellow
	    Get-Content $_.FullName | select -Skip 5  -First 1000 | ConvertFrom-Csv -Header $LogHeader -Delimiter "," |
	    select connector-id,local-endpoint,@{n='remote-endpoint';e={(($_.'remote-endpoint' -split ":")[0])}},date-time |
	    foreach -Process {
...
$RptHash.Keys | foreach { $RptEnt = $_; $RptEnt | ConvertFrom-Csv -Header @("Connector", "Local-Endpoint", "Remote-Endpoint") | 
Select Connector, Local-EndPoint, Remote-EndPoint,
