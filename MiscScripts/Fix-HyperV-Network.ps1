function Fix-HyperV
{
    ...
    New-NetIPAddress -InterfaceAlias "vEthernet (Default Switch)" -IPAddress "192.168.151.1" -PrefixLength 24
    Start-Sleep -Seconds 5

    if (-not (Get-NetNat).Active) {
        New-NetNAT -Name HVNAT -InternalIPInterfaceAddressPrefix 192.168.151.0/24
        Start-Sleep -Seconds 5
    } else {
